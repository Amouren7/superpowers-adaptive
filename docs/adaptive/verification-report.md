...
对照：adaptive.1（`f01828a`）、上游 v6.3.0（`b36e082`）、本轮最终提交 `77270e9`（含 `803d75f`）
...
- 最终交付包 a3 被实跑覆盖的场景：**T03、T07**（v3 判据）。
- 其余 7 个场景（T01/T02/T04/T05/T06/T08/T09）跑在 a2 上。a2 与 a3 的全部 51 个技能文件中**只差 `executing-plans/SKILL.md` 一个文件**，而这 7 个场景的加载集合不含该技能（T05/T08 由 prompt 显式指定 `systematic-debugging`；T01/T02/T04/T06/T09 走入口 + 各自技能，入口文本两版相同）。
- 为控制评测预算，**没有**把这 7 个场景在 a3 上重跑。要补全覆盖，一条命令即可（约 9 次 headless 运行）：
  ```
