...
test('startup context injects the bootstrap as one user message until agent_end', async () => {
  ...
  assert.match(textOf(result.messages[0]), /Classify this task first/);
  ...
  assert.match(textOf(repeatedProviderRequest.messages[0]), /Classify this task first/);
...
test('session_compact injects bootstrap after compaction summaries, not before compaction', async () => {
  ...
  assert.match(textOf(result.messages[1]), /Classify this task first/);
